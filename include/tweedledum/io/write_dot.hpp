...
{
	layers_view network_view(dag_network);
	...
	for (auto level = network_view.num_layers(); level-- > 1;) {
		...
		os << "\t\trank = same;\n";
		os << fmt::format("\t\tLevel{};\n", level);
		network_view.foreach_gate([&network_view, &os, level](auto const& node, auto index) {
	...
	network_view.foreach_gate([&network_view, &os](auto const& node, auto index) {
		network_view.foreach_child(node, [&](auto child) {
			os << fmt::format("\tNode{} -> Node{} [style = solid];\n", index, child.index);
		});
	});
	network_view.foreach_output([&network_view, &os](auto const& node, auto index) {
		network_view.foreach_child(node, [&](auto child) {
			os << fmt::format("\tNode{} -> Node{} [style = solid];\n", index, child.index);
		});
	});
