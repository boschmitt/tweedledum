...
inline void write_dot(Network const& dag_network, std::string filename = "test.dot")
{
	...
	dag_network.foreach_output([&file](auto const& node, auto index) {
		node.gate.foreach_target([&file, &index](auto qid) {
			fprintf(file, "\t\tNode%d [label = \"%d\"", index, qid);
	...
	ntk_lvl.foreach_gate([&ntk_lvl, &file](auto const& node, auto index) {
	...
	ntk_lvl.foreach_output([&ntk_lvl, &file](auto const& node, auto index) {
