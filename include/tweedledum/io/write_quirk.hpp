...
public:
	quirk_json_builder(uint32_t num_qubits)
	...
	friend std::ostream& operator<<(std::ostream& out, quirk_json_builder builder)
	{
		...
		out << map_and_join(builder.columns_.begin(), builder.columns_.end(),
...
private:
	void new_column()
	{
		columns_.emplace_back(num_qubits_, "1");
	}

	bool is_last_column_empty()
	{
		auto element = std::find_if(columns_.back().begin(), columns_.back().end(),
