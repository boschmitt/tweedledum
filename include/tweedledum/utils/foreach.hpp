...
/* \brief Applies a function to elements satisfying specific criteria in a range of elements
 *
 * The parameter ``predicate`` is any callable that have the following signature.
 * - ``bool(element)``
 *
 * The parameter ``fn`` is any callable that must have one of the following four signatures.
 * - ``void(element)``
...
Iterator foreach_element_if(Iterator begin, Iterator end, Pred&& predicate, Fn&& fn,
