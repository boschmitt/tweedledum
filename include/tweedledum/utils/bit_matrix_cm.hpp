...
#pragma region Element access
	...
	constexpr auto row(std::vector<uint32_t> const& columns, size_type row_index) const
	{
		row_type value(columns.size(), 0);
		for (size_type column_index = 0; column_index < columns.size(); ++column_index) {
...
#pragma region Iterators
	...
	{
		foreach_element(storage_.lines_.cbegin(), storage_.lines_.cend(), fn, 0);
	}
...
#pragma region Debug
	...
	void print_debug(std::vector<uint32_t> const& columns, std::ostream& out = std::cout) const
	{
		for (size_type row_index = 0; row_index < num_rows(); ++row_index) {
