...
public:
    ...
    //implementation of meet function (modeled after join and Knuth book description)
    
    node meet(node f, node g)
    {
        //double check these below, but i think they make sense...
        //using pg 119 of book chapter
        if (f == 0)
            return f;
        if (g == 0)
            return g;
        ...
        //find if operation has already been performed
        const auto it = compute_table.find({f, g, zdd_meet});
        if (it != compute_table.end()) {
            assert(!nodes[it->second].dead);
            return it->second;
        }
        ...
        if(F.var > G.var)
        {
            r_hi = union_(G.lo,G.hi);
            r = meet(f,r_hi);
            auto const& R = nodes[r];
            r_lo = R.lo;
            r_hi = R.hi;
        ...
        //otherwise proceed analogously to join operation but where l <-> h
        else {
            r_hi = meet(F.hi, G.hi);
            const auto r_hl = meet(F.hi, G.lo);
            const auto r_lh = meet(F.lo, G.hi);
            const auto r_ll = meet(F.lo, G.lo);
            r_lo = union_(r_hl, union_(r_lh, r_ll));
        }
        
        const auto var = std::min(F.var, G.var);
        return compute_table[{f, g, zdd_meet}] = unique(var, r_lo, r_hi);
...
public:
	...
	void run()
	{
        ...
        std::vector<std::vector<std::vector<std::string>>> phi_map_trans_pairs;
        
        for(uint32_t i = 0; i < found_sets_index_barriers.size()-2; i++)
        {
            
            //add new section of phi_map_trans_pairs for current phi1 and phi2
            phi_map_trans_pairs.push_back(std::vector<std::vector<std::string>>());
            ...
            int phi_map_trans_pairs_counter = 0;
            ...
            for(auto const& j : phi_1)
            {
                ...
                for(auto const& k : phi_2)
                {
                    ...
                    if((check1 == true) && (check2 ==true) && (check3 == true))
                    {
                        ...
                        //add transition to phi_map_trans_pairs
                        //and phi_map_trans_pairs_counter++
                        phi_map_trans_pairs[i].push_back(std::vector<std::string>());
                        phi_map_trans_pairs[i][phi_map_trans_pairs_counter].push_back(j);
                        phi_map_trans_pairs[i][phi_map_trans_pairs_counter].push_back(k);
                        
                        phi_map_trans_pairs_counter++;
...
//        for(auto const& item : phi_map_trans_pairs)
//        {
//            for(auto const& thing: item)
//            {
//                int temp_count = 0;
//                for (auto const& thing2: thing)
//                {
//                    std::cout <<temp_count << " "<< thing2 << "\n";
//                    temp_count++;
//                }
//            }
//        }

        std::cout << phi_map_trans_pairs.size() << "\n";
        std:: cout << phi_map_trans_pairs[0].size() << "\n";
        std:: cout << phi_map_trans_pairs[1][0].size() << "\n";
        
        std::vector<std::string> full_paths;
        
        for(uint32_t i = 0; i < phi_map_trans_pairs.size()-1; i++)
        {
            for(uint32_t j = 0; j < phi_map_trans_pairs[i].size(); j++)
            {
                for(uint32_t k = 0; k < phi_map_trans_pairs[i+1].size(); k++)
                {
                    if(phi_map_trans_pairs[i][j][1] == phi_map_trans_pairs[i+1][k][0])
                    {
                        if(i==0)
                        {
                            std::string temp_path =phi_map_trans_pairs[i][j][0]+"| "+ phi_map_trans_pairs[i][j][1] ;
                            
                            //std::cout << temp_path << "\n";
                            full_paths.push_back(temp_path);

                        }
                        if(i!=0)
                        {
                            uint32_t len_of_match = phi_map_trans_pairs[i][j][0].size();
                            std::cout << "len. of match"<< len_of_match << "\n";
                            //stopped here. at this point, we want to go through what is saved in full_paths.
                            //check the last "len_of_match" chars for a match with the first part of a swap pair
                            //if match, append phi_map_trans_pairs[i][j][1] to that index in full_paths
                            //but make sure to create a copy to not block that path for others. then minus one
                            //from indexer and break to next iteration of loop controlled by k
                        }
                   }
                }
            }
...
//        std::vector<std::vector<std::string>> full_paths;
//        int temp_count = 0;
//
//        for(uint32_t i = 0; i < phi_map_trans_pairs.size(); i++)
//        {
//            //first add all beginning paths to full_paths, then build from there
//            if(i == 0)
//            {
//
//                for(uint32_t j = 0; j < phi_map_trans_pairs[i].size(); j++ )
//                {
//
//                    full_paths.push_back(std::vector<std::string>());
//                    full_paths[temp_count].push_back(phi_map_trans_pairs[i][j][0]);
//                    full_paths[temp_count].push_back(phi_map_trans_pairs[i][j][1]);
//
//                    temp_count++;
//
//                }
...
//            if(i>0)
//            {
//                std::vector<std::vector<std::string>> full_paths2 = full_paths;
//
//                for(uint32_t j = 0; j < phi_map_trans_pairs[i].size(); j++ )
//                {
//                    for(uint32_t k = 0; k < full_paths.size(); k++)
//                    {
//
//
//                        if(full_paths[k][1] == phi_map_trans_pairs[i][j][0])
//                        {
//                            full_paths2[temp_count].push_back(full_paths[k][0]);
//                            full_paths2[temp_count].push_back(full_paths[k][1]);
//                            temp_count++;
//
//                            full_paths2[k].push_back(phi_map_trans_pairs[i][j][1]);
//
//                        }
//
//                    }
//                    full_paths = full_paths2;
//
//                }
//            }
//        }
//
//        std::cout<<"full paths:" << full_paths.size() << " " << full_paths[0].size() << "\n";
//        std::cout<<"full paths:" << full_paths.size() << " " << full_paths[1].size() << "\n";
//        std::cout<<"full paths:" << full_paths.size() << " " << full_paths[2].size() << "\n";
//
//        std::cout << "test here " <<full_paths[0].back()<<"\n";
//
//
