...
public:
	...
	mapping_view<Network> run()
	{
		...
		uint32_t max_coverage_idx = std::max_element(map_coverage_.begin(),
		                                             map_coverage_.end())
		                            - map_coverage_.begin();
		...
		mapped_ntk.set_virtual_phy_map(current_mapping);
			...
			if (count_2q == index_of_swap[index_counter] && (count_2q < (map_start_[max_coverage_idx] + map_coverage_[max_coverage_idx]))) {
		...
		stats_.time_total = stats_.time_swap_layers_build + stats_.time_map_search
		                    + stats_.time_new_network;
		return mapped_ntk; 
	}
...
private:
		...
		depth_nkt.foreach_gate([&](auto const& n, auto node_index) {
			...
			for (uint32_t i = 0; i < swap_layers_.size(); i++) {
				...
				if (mp == zdd_.bottom()) {
					...
					// fmt::print("{}, {}\n", zdd_.count_sets(mp), zdd_.count_sets(m_prime));
				}
	...
	{
		bill::zdd_base zdd_swap_layers(arch_.num_edges());
		...
		for (auto i = 0u; i < arch_.num_edges(); ++i) {
