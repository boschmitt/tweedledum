...
struct Control {
	uint32_t bit;
	bool polarity = true;
	Control(uint32_t bit, bool polarity)
	    : bit(bit)
	    , polarity(polarity)
	{}
};
...
template<class Network>
void add_equivalent_to_net_property1(Network& net, const kitty::cube& a, const kitty::cube& b,
...
template<class Network>
void add_equivalent_to_net_property2(Network& net, const kitty::cube& a, const kitty::cube& b,
                                     std::vector<uint32_t> const& qubit_map)
{
...
template<class Network>
void opt_stg_from_esop(Network& net, optimized_esop o_esop, std::vector<uint32_t> const& qubit_map)
{
	...
	for (uint64_t i = index; i < o_esop.cubes.size(); i++) {
