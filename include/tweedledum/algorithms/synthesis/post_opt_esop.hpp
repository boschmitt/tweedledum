...
namespace tweedledum {
	...
	void add_gate_with_neg_contr(Network& net, gate_kinds_t gate_type, std::vector<Control> controls, std::vector<uint32_t> target)
	{
	...
	template<class Network>
	void from_cube_to_toffoli(Network& net, const kitty::cube& cube, const std::vector<uint8_t>& qubit_map)
	{
		...
		for (auto v = 0; v < 32; ++v) {
			if (mask & 1) 
				controls.push_back((bits & 1)? Control(qubit_map[v], true): Control(qubit_map[v], false));
	...
	optimized_esop match_pairing (std::vector<kitty::cube> esop)
	{
	...
	template<class Network>
	void add_equivalent_to_net_property1(Network& net, const kitty::cube& a, const kitty::cube& b, std::vector<uint8_t> const& qubit_map)
	{
		...
		for ( unsigned i = 0; i < 32; i++ )
		{
			if ( ( to_create & 1 ) == 1 )
				controls.push_back(
					(!( ((other_cube._bits & ( 1 << i )) >> i) == 1))? Control(qubit_map[i], false) : Control(qubit_map[i], true));
	...
	template<class Network>
	void add_equivalent_to_net_property2 (Network& net, const kitty::cube& a, const kitty::cube& b, std::vector<uint8_t> const& qubit_map)
	{
			...
			if ( !done )
			{
				...
				for ( int j = 0; j < 32; ++j )
				{
					if( (((b._mask >> j) & 1) == 1) && (j!=i) )							
										common_controls.push_back(b._bits >> j == 1 ? Control(qubit_map[j], true) : Control(qubit_map[j], false));
				...
				for ( int j = 0; j < 32; ++j )
				{
					if( (((a._mask >> j) & 1) == 1) && (j!=i) )
										common_controls.push_back(a._bits >> j == 1 ? Control(qubit_map[j], true) : Control(qubit_map[j], false));
				}
				}
			}
			else
	...
	template<class Network>
	void opt_stg_from_esop (Network& net, optimized_esop o_esop, std::vector<uint8_t> const& qubit_map)
	{
		/* foreach elem into pairing, put the first two cubes into a function that gets the eq circuit and 
		add the gate to the network */
		uint index = 0;
