...
{
    int count_set( const uint16_t bits ) { return __builtin_popcount( bits ); }

    int num_variables(const std::vector<kitty::cube>& esop)
    {
        int16_t var_mask;
    ...
    struct optimized_esop
	{
		...
		public:
		optimized_esop(std::vector<kitty::cube> cubes, std::vector<int> pairing ) : cubes(cubes), pairing(pairing) {}
        
        uint get_num_pairs = pairing.size();
		void print()
		{
			std::cout << "Ordered cubes for optimized esop:"<< std::endl;
			for(auto cube : cubes){ cube.print(5);std::cout << std::endl;} 
			for(auto type : pairing) {std::cout << type << " ";} std::cout << std::endl;
		}
	};
    ...
    class optimization_graph
    {
        ...
        struct edge
        {
            ...
            edge(int from, int to, uint weight, int type) : from(from), to(to), weight(weight), type(type) {}
        ...
        {
            /* whether two esop have some common controls, and one of them has a unique control that is not present
            in the other --> we have a gain only if there are some independent controls */
            if (( ( ( f._mask & s._mask ) & ( f._bits ^ s._bits ) ) == 0 ) && ( ( f._mask & s._mask ) != 0 ) )
            {
                auto a_notb = f._mask & ( ~s._mask );
                auto b_nota = ( ~f._mask ) & s._mask;

                int old_cost = t_cost(f.num_literals(), num_var) + t_cost(s.num_literals(), num_var);
                if (count_set( a_notb ) == 1 )
                {
                    return old_cost - ( 2*t_cost(count_set(b_nota), num_var)+t_cost(f.num_literals(), num_var) );
                }

                if (count_set( b_nota ) == 1 )
                {

                    return old_cost - ( 2*t_cost(count_set(a_notb), num_var )+t_cost(s.num_literals(), num_var) );
                }
                return 0;
    ...
    public:
        ...
        optimized_esop match_properties()
        {
            ...
            for (auto& edge : edges)
            {
                if ( (std::find(m_nodes.begin(), m_nodes.end(), edge.from) == m_nodes.end()) &&
                      (std::find(m_nodes.begin(), m_nodes.end(), edge.to) == m_nodes.end()) )
                {
                    edge.match();
        ...
        optimization_graph(std::vector<kitty::cube> esop) 
        : esop(esop)
        {
