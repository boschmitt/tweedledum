...
void linear_synth_binary(Circuit& circuit, std::vector<wire::id> const& qubits, parity_terms<uint32_t> parities)
{
	...
	for (auto i = 0u; i < num_qubits; ++i) {
		...
		if (rotation_angle != 0.0) {
			circuit.create_op(gate_lib::rz(rotation_angle), qubits[i]);
		}
	}
	...
	for (auto i = 1u; i < (1u << num_qubits); i++) {
		...
		for (auto j = 0u; j < num_qubits; j++) {
			if ((first_num != j) && ((qubits_states[j] ^ qubits_states[first_num]) == i)) {
				...
				circuit.create_op(gate_lib::cx, qubits[j], qubits[first_num]);
				auto rotation_angle = parities.extract_term(qubits_states[first_num]);
				if (rotation_angle != 0.0) {
					circuit.create_op(gate_lib::rz(rotation_angle),
					                  qubits[first_num]);
				}
			}
		}
	...
	for (int i = num_qubits - 1; i > 0; i--) {
		circuit.create_op(gate_lib::cx, qubits[i - 1], qubits[i]);
	}
}

template<class Circuit>
void linear_synth_gray(Circuit& circuit, std::vector<wire::id> const& qubits, parity_terms<uint32_t> parities)
{
	const auto num_qubits = qubits.size();
	...
	for (auto i = num_qubits - 1u; i > 0; --i) {
		for (auto j = (1u << (i + 1)) - 1u; j > (1u << i); --j) {
			...
			circuit.create_op(gate_lib::cx, qubits[temp], qubits[i]);
			auto rotation_angle = parities.extract_term(qubits_states[i]);
			if (rotation_angle != 0.0) {
				circuit.create_op(gate_lib::rz(rotation_angle), qubits[i]);
			}
 ...
 *
 * This is the in-place variant of ``linear_synth``, in which the circuit is passed as a parameter
 ...
 */
template<class Circuit>
Circuit linear_synth(uint32_t num_qubits, parity_terms<uint32_t> const& parities,
