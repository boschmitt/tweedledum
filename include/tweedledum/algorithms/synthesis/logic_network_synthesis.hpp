...
public:
	bennett_mapping_strategy(LogicNetwork const& ntk)
	{
		...
		view.foreach_node([&](auto n) {
			...
			/* compute step */
			it = steps.insert(it, {n, mapping_strategy_action::compute});
			++it;

			if (!drivers.count(n))
				it = steps.insert(it, {n, mapping_strategy_action::uncompute});
	...
	void run()
	{
		...
		strategy.foreach_step([&](auto node, auto action) {
			...
			case mapping_strategy_action::compute: {
				if (ps.verbose)
					std::cout << "[i] compute " << ntk.node_to_index(node)
					          << "\n";
			...
			case mapping_strategy_action::uncompute: {
				if (ps.verbose)
					std::cout << "[i] uncompute " << ntk.node_to_index(node)
					          << "\n";
	...
	void compute_node(mt::node<LogicNetwork> const& node, uint32_t t)
	{
		...
		if constexpr (mt::has_is_xor3_v<LogicNetwork>) {
			if (ntk.is_xor3(node)) {
				...
				compute_xor3(node_to_qubit[ntk.index_to_node(controls[0] >> 1)],
				             node_to_qubit[ntk.index_to_node(controls[1] >> 1)],
		...
		if constexpr (mt::has_is_maj_v<LogicNetwork>) {
			if (ntk.is_maj(node)) {
				auto controls = get_fanin_as_literals<3>(node);
				compute_maj(node_to_qubit[ntk.index_to_node(controls[0] >> 1)],
				            node_to_qubit[ntk.index_to_node(controls[1] >> 1)],
				            node_to_qubit[ntk.index_to_node(controls[2] >> 1)],
