                           ...
                           (gg_network, netlist), (mcmt_gate, io3_gate))
{
	...
	SECTION("T gate")
	{
		network.add_gate(gate_base(gate_set::rotation_z, angles::pi_quarter), qubit);
		TestType result = identify_rz(network);
		CHECK(result.num_qubits() == 1);
		CHECK(result.num_gates() == 1);
		auto& node = result.get_node(1u);
		CHECK(node.gate.is(gate_set::t));
	}
	SECTION("S gate (phase)")
	{
		network.add_gate(gate_base(gate_set::rotation_z, angles::pi_half), qubit);
		TestType result = identify_rz(network);
		CHECK(result.num_qubits() == 1);
		CHECK(result.num_gates() == 1);
		auto& node = result.get_node(1u);
		CHECK(node.gate.is(gate_set::phase));
	}
	SECTION("Pauli Z gate")
	{
		network.add_gate(gate_base(gate_set::rotation_z, angles::pi), qubit);
		TestType result = identify_rz(network);
		CHECK(result.num_qubits() == 1);
		CHECK(result.num_gates() == 1);
		auto& node = result.get_node(1u);
		CHECK(node.gate.is(gate_set::pauli_z));
	}
	SECTION("T† gate (Conjugate transpose)- negative rotation")
	{
		...
		CHECK(result.num_gates() == 1);
		auto& node = result.get_node(1u);
		CHECK(node.gate.is(gate_set::t_dagger));
	}
	SECTION("S† gate (Conjugate transpose) - negative rotation")
	{
		...
		CHECK(result.num_gates() == 1);
		auto& node = result.get_node(1u);
		CHECK(node.gate.is(gate_set::phase_dagger));
	}
	SECTION("T† gate (Conjugate transpose)- positive rotation")
	{
		angle rotation_angle(7, 4);
		network.add_gate(gate_base(gate_set::rotation_z, rotation_angle), qubit);
		TestType result = identify_rz(network);
		CHECK(result.num_qubits() == 1);
		CHECK(result.num_gates() == 1);
		auto& node = result.get_node(1u);
	...
	SECTION("P + T")
	{
		network.add_gate(gate_base(gate_set::rotation_z, angles::pi_quarter + angles::pi_half), qubit);
		TestType result = identify_rz(network);
		CHECK(result.num_qubits() == 1);
	...
	SECTION("Z + T")
	{
		network.add_gate(gate_base(gate_set::rotation_z, angles::pi_quarter + angles::pi), qubit);
		TestType result = identify_rz(network);
		CHECK(result.num_qubits() == 1);
		CHECK(result.num_gates() == 2);
		auto& node = result.get_node(1u);
