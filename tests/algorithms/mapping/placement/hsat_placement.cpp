...
*------------------------------------------------------------------------------------------------*/
#include "tweedledum/algorithms/mapping/placement/hsat_placement.hpp"
...
TEMPLATE_PRODUCT_TEST_CASE("Test for SAT intial placement heuristic", "[hsat_placement][mapping]",
                           (netlist, op_dag), (w3_op, wn32_op))
{
	TestType network;
	SECTION("Empty network") {
		device device = device::path(network.num_qubits());
		std::vector<wire::id> placement = detail::hsat_placement(network, device);
