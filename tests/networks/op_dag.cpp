                           ...
                           (op_dag), (w3_op, wn32_op))
{
	...
	SECTION("Input iterator") {
		...
		i = 0u;
		network.foreach_input([&] (node_type const& node, node::id const id) {
	...
	SECTION("Output iterator") {
		...
		i = 0u;
		network.foreach_output([&] (node_type const& node, node::id const id) {
			CHECK(node.op.is(gate_ids::input));
			CHECK(id == i);
			++i;
		});
		...
		node::id n = network.create_op(gate_lib::ncx, q0, q1, q2);
		network.foreach_output([&] (node_type const& node, node::id const id) {
                           ...
                           (op_dag), (w3_op, wn32_op))
{
	...
	SECTION("One operation") {
		...
		node::id n1 = network.create_op(gate_lib::cx, q1, q2);
		node::id n2 = network.create_op(gate_lib::cx, q2, q0);
