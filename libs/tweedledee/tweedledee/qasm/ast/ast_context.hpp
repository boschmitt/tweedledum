...
inline void* operator new(size_t bytes, tweedledee::qasm::ast_context const& ctx,
                          size_t alignment = 8)
{
	return ctx.allocate(bytes, alignment);
}

inline void operator delete(void* ptr, tweedledee::qasm::ast_context const& ctx, size_t)
{
	ctx.deallocate(ptr);
}
