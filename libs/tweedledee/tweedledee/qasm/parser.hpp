...
public:
	parser(preprocessor& pp_lexer, semantic& semantic, source_manager& source_manager, diagnostic_engine& diagnostic)
	    : pp_lexer_(pp_lexer)
	    , semantic_(semantic)
		...
		while (!error_) {
			...
			case token_kinds::kw_creg:
				semantic_.on_register_declaration(
				    parse_register_declaration(register_type::classical));
				break;

			case token_kinds::kw_qreg:
				semantic_.on_register_declaration(
				    parse_register_declaration(register_type::quantum));
				break;
...
private:
	...
	bool parse_argument_list(stmt_gate::builder& builder)
	{
		do {
	...
	std::unique_ptr<stmt_gate> parse_gate_statement()
	{
		...
		if (!parse_argument_list(stmt_builder)) {
			return nullptr;
		}
		expect_and_consume_token(token_kinds::semicolon);
		if (not error_) {
	...
	//           | creg <id> [ <nninteger> ] ;
	//
	std::unique_ptr<decl_register> parse_register_declaration(register_type type)
	{
