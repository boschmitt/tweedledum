...
inline void* operator new(size_t bytes, tweedledee::quil::ast_context const& ctx,
                          size_t alignment = 8)
{
	return ctx.allocate(bytes, alignment);
}

inline void operator delete(void* ptr, tweedledee::quil::ast_context const& ctx, size_t)
{
	ctx.deallocate(ptr);
}
