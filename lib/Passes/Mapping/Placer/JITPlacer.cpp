...
std::vector<WireRef> JITPlacer::find_free_phy() const
{
...
void JITPlacer::place_two_v(WireRef const v0, WireRef const v1)
{
...
void JITPlacer::place_one_v(WireRef v0, WireRef v1)
{
    WireRef phy0 = state_.v_to_phy.at(v0);
    WireRef phy1 = state_.v_to_phy.at(v1);
    std::vector<WireRef> const free_phy = find_free_phy();
...
void JITPlacer::add_swap(WireRef const phy0, WireRef const phy1)
{
    num_swaps_ += 1;
...
double JITPlacer::compute_cost(std::vector<WireRef> const& v_to_phy, std::vector<InstRef> const& layer)
{
    double cost = 0.0;
